report 50020 "Sales Order Confirmation"
{
    ...
    // NUB1.04,10/03/18,SK: Added new column as "Qty. Per Pallets" into layout.
    // NUB1.05,01/18/19,SK: Added code get Lot Tracking Details & changed the layout to add LOT details & delcared global variables.
    // NUB1.06,03/12/19,SK: Added "Total Case Count" totals in layout & delcared global variables.
    // NUB1.07,09/27/19,ST: Modifications to format the totals and print the value with dollar sign.
    //                       - Modified the format settings and used regional settings to print thousands seperator and dollar sign.
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports/Layout/SalesOrderConfirmation.rdl';
...
#pragma warning restore AL0432
                    ...
                    column(Qty_Pallets_Caption; Qty_Pallets_CaptionLbl)
                    {
                    }
                    dataitem(SalesLine; "Integer")
                    {
                        DataItemTableView = SORTING(Number);
                        column(TempSalesLine_Qty_Per_Pallet; TempSalesLine."Qty.  Per Pallet")
                        {
                        }
                        ...
                        column(TotCaseCountVarGbl; TotCaseCountVarGbl)
                        {
                        }
                        dataitem(TrackingSpec; "Integer")
                        {
                            DataItemTableView = SORTING(Number);
                            column(TrackingSpec_Number; Number)
                            {
                            }
                            column(Lot_No_ReservEntry; ReservEntryRecGbl."Lot No.")
                            {
                            }
                            column(Qty_Base_ReservEntry; -1 * ReservEntryRecGbl."Quantity (Base)")
                            {
                            }
                            ...
                            begin
                                //NUB1.05 Start
                                IF Number = 1 THEN
                                    ReservEntryRecGbl.FINDSET
                                ELSE
                                    ReservEntryRecGbl.NEXT;
                                //NUB1.05 End
                            end;
                            ...
                            begin
                                //NUB1.05 Start
                                IF NOT ShowLotDet THEN
                                    CurrReport.BREAK;

                                SETRANGE(Number, 1, ReservEntryRecGbl.COUNT);
                                //NUB1.05 End
                            end;
                        }
                        ...
                        begin
                            ...
                            //NUB1.05 Start
                            ReservEntryRecGbl.RESET;
                            ReservEntryRecGbl.SETRANGE("Source Type", 37);
                            ReservEntryRecGbl.SETRANGE("Source Subtype", 1);
                            ReservEntryRecGbl.SETRANGE("Source ID", TempSalesLine."Document No.");
                            ReservEntryRecGbl.SETRANGE("Source Ref. No.", TempSalesLine."Line No.");
                            IF ReservEntryRecGbl.FINDSET THEN;
                            //NUB1.05 ENd

                            //NUB1.06 Start
                            IF TempSalesLine.Type = TempSalesLine.Type::Item THEN
                                TotCaseCountVarGbl += TempSalesLine.Quantity;
                            //NUB1.06 End
                            //NA0007.Begin
                ...
                begin
                    ...
                    CLEAR(TotCaseCountVarGbl); //NUB1.06
                end;
            ...
            begin
                IF PrintCompany THEN BEGIN
                    ...
                    END;
                END;
                CurrReport.LANGUAGE := EventsAndSubcribers.GetLanguageID("Language Code");
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field("Show Lot Details"; ShowLotDet)
                    {
                        ApplicationArea = All;
                    }
                }
    ...
    var
        ...
        ItemSKU: Code[50];
        Qty_Pallets_CaptionLbl: Label 'Qty.Per Pallets';
        "-NUB1.05-": Integer;
        ReservEntryRecGbl: Record "Reservation Entry";
        ShowLotDet: Boolean;
        "-NUB1.06-": Integer;
        TotCaseCountVarGbl: Decimal;
